...
describe('Payment group service', () => {
  ...
  it('getDiscontinuedFrFeature should always return true (ff4j removed, feature permanently enabled)', async () => {
    const response = await paymentGroupService.getDiscontinuedFrFeature();
  ...
  it('getBSFeature should always return true (ff4j removed, feature permanently enabled)', async () => {
    const response = await paymentGroupService.getBSFeature();
    expect(response).toBe(true);
  });
