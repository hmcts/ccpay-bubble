...
module.exports = (security, appInsights) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    Logger.getLogger(`response: ${process.env.NODE_ENV}`).info(process.env.NODE_ENV);

    app.use('/api', (req, res, next) => {
      Logger.getLogger(`devarea: ${req}`).info(res);
  ...
  app.get('**', security.protectWithAnyOf(roles.allRoles, ['/assets/']), csrfProtection,
    (req, res) => {
      Logger.getLogger(`response: ${req} -> error`).info(res);
